...
const LocaleMenuButton: React.FC<{ isTextHidden }> = ({ isTextHidden }) => {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left z-10 w-full">
      <div>
        ...
        >
          <div
            className="flex gap-2 hover:bg-brand-500 pl-1 py-1 xs:py-2 rounded w-full items-center"
            onClick={() => {}}
            role="button"
            onKeyDown={() => {}}
            tabIndex={0}
            aria-label={'Locale Changer'}
            aria-hidden={false}
            aria-pressed={false}
          >
            <GlobeAmericasIcon className="w-4 text-brand-300" />
            <div
      ...
      >
        <Menu.Items className="absolute top-0 mt-2 w-44 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ">
...
export default LocaleMenuButton
