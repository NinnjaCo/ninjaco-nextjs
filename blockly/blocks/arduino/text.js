...
Blockly.Blocks['text_join'] = {
  ...
  decompose: function (workspace) {
    const containerBlock = Blockly.Block.obtain(workspace, 'text_create_join_container')
    containerBlock.initSvg()
    let connection = containerBlock.getInput('STACK').connection
    for (let i = 0; i < this.itemCount_; i++) {
      const itemBlock = Blockly.Block.obtain(workspace, 'text_create_join_item')
      itemBlock.initSvg()
...
Blockly.Blocks['text_getSubstring'] = {
  ...
  updateAt_: function (n, isAt) {
    ...
    const menu = new Blockly.FieldDropdown(this['WHERE_OPTIONS_' + n], function (value) {
      const newAt = value == 'FROM_START' || value == 'FROM_END'
      // The 'isAt' variable is available due to this function being a closure.
      if (newAt != isAt) {
        const block = this.sourceBlock_
...
Blockly.Blocks['text_prompt'] = {
  ...
  init: function () {
    ...
    this.setOutput(true, 'String')
    // Assign 'this' to a variable for use in the tooltip closure below.
    const thisBlock2 = this
    this.setTooltip(function () {
      return thisBlock2.getFieldValue('TYPE') == 'TEXT'
