...
class JSONGenerator extends Blockly.Generator {
  PRECEDENCE = 0
  constructor() {
    super('JSON')
  }
  ...
  logic_null(block: Blockly.Block) {}
  text(block: Blockly.Block) {}
  math_number(block: Blockly.Block) {}
  logic_boolean(block: Blockly.Block) {}
  member(block: Blockly.Block) {}
  lists_create_with(block) {}
  object(block: Blockly.Block) {}
}
...
jsonGenerator['object'] = function (block) {
  const statementMembers = jsonGenerator.statementToCode(block, 'MEMBERS')
  const code = '{\n' + statementMembers + '\n}'
  return [code, jsonGenerator.PRECEDENCE]
}
